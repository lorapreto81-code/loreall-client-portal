...
export const PlanCard = ({ customer, days, onRenewClick }: PlanCardProps) => {
  ...
  return (
    <div className="card-elevated p-5 relative overflow-hidden">
      ...
      <div className="mt-6 p-4 rounded-2xl bg-muted/30 border border-white/5 space-y-4">
        <p className="text-[13px] leading-relaxed text-foreground/90">
          Isso é inesperado — o card não devia ficar assim depois do que você descreveu. Se a assinatura "ativa" ficou vinculada ao ID certo, o card deveria mostrar "Assinatura ativa" com o botão de cancelar, não voltar pro botão padrão de "Renovar acesso" (esse botão específico só aparece quando o sistema não acha <strong>nenhuma</strong> assinatura pra esse cliente).
        </p>
        ...
        <div className="bg-black/40 rounded-xl p-3 font-mono text-[11px] text-primary/90 overflow-x-auto border border-white/5">
          <pre>
...
where customer_id = 1590013 or customer_name ilike '%josé%lourenco%';`}
        ...
        <p className="text-[13px] leading-relaxed text-foreground/90">
          E, se puder, mais uma coisa junto (me ajuda a cortar caminho, evita mais uma rodada): abre o DevTools → Network → dá refresh na Área do Cliente → acha a chamada pra <code className="text-primary">syncpay-subscription-status</code> → me manda o corpo da resposta. Isso mostra exatamente o que a nossa function está enxergando <em>agora</em>, direto — se ela não está achando nenhuma linha, ou se está achando e o status não é o que a gente espera.
        </p>
